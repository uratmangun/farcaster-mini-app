 ...
 *   FARCASTER_DOMAIN - Your domain for automatic farcaster.json updates (optional)
 *
 * Usage:
 ...
 * @param {string} domain - The FARCASTER_DOMAIN to use
...
async function generateImage() {
  try {
    ...
    // Check for FARCASTER_DOMAIN environment variable (loaded from .env file via dotenv)
    const farcasterDomain = process.env.FARCASTER_DOMAIN;
    if (!farcasterDomain) {
      console.warn('⚠️  Warning: FARCASTER_DOMAIN is not set.');
      console.warn('   The farcaster.json file will not be automatically updated with the correct domain.');
      console.warn('   Add FARCASTER_DOMAIN=your-domain.com to your .env file to enable automatic configuration.');
    } else {
    ...
    if (process.argv[2] === '--help' || process.argv[2] === '-h') {
      ...
      console.log('   FARCASTER_DOMAIN=your-domain.com       (optional)');
    ...
    console.log('\n⏳ Generating images... This may take a few moments.');
    ...
    if (farcasterDomain) {
      ...
      console.log(`   1. Set FARCASTER_DOMAIN in your .env file`);
